...
import com.wangin.admin.entity.*;
import com.wangin.admin.predicate.*;
...
//    }
    ...
    public String golftest3(HttpServletRequest request, Model model,
        ...
        Optional<GolfUserEntity> aaa = golfUserRepository.findOne(res1Predicate.search(sessioninid));
        Long uino = aaa.get().getUI_NO();
        System.out.println(uino);
        Optional<CaEntity> bbb = caRepository.findOne(res2Predicate.search(uino, mountin));
        Long cano = bbb.get().getCA_NO();
        String caid = bbb.get().getCA_ID();
        String capw = bbb.get().getCA_PASSWORD();
        System.out.println(caid);
        GolfDto golfDto = new GolfDto(null, cano, uino, mountin, caid, capw, hope_y, hope_m, hope_d, hope_t1, hope_t2, hope_h, hope_c, 0, 0, 0, 0, null, null, null);
